...
export class CheckItemService {
  ...
  async findAll(checklist_id) {
    const checkItems = await this.checkItemRepository
      ...
      .leftJoinAndSelect('check_item.check_item', 'check_item')
      .getRawMany();
  ...
  async moveCheckItemBlock(id: number, to: number) {
    ...
    try {
      ...
      const currentCheckItems = await this.checkItemRepository
        ...
        .where(
          'checklist.lists_order >= :min AND checklist.lists_order <= :max',
          {
            min: min,
            max: max,
          },
  ...
  async remove(id: number) {
    ...
    const list = (await this.verifyListById(id)).checkItem[0];
    const checklist_id = list.checklist_id;
